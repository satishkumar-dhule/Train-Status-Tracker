...
//! behavior — order preserved, RailRadar gated by key — on pre-parsed names.
...
fn returns_an_empty_list_when_everything_is_unknown() {
    let providers =
        build_status_providers(Arc::new(MockTransport::new()), &names(&["bogus"]), None);
...
fn an_empty_list_yields_an_empty_provider_list() {
    let providers = build_status_providers(Arc::new(MockTransport::new()), &[], None);
    assert!(providers.is_empty());
}
