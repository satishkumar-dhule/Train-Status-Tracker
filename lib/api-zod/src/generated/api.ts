...
export const HealthCheckResponse = zod.object({
  "status": zod.string(),
  "redis": zod.enum(['up', 'down', 'disabled']).optional().describe('Shared Redis cache availability'),
  "uptime_seconds": zod.number().int().optional().describe('Process uptime in whole seconds'),
  "version": zod.string().nullish().describe('SERVICE_VERSION env value'),
  "timestamp": zod.string().optional().describe('ISO-8601 generation time')
}).describe('Readiness payload; the endpoint always returns 200 while the process is up, and dependency state is reported per-check for diagnostics.')
 ...
 */
export const searchTrainsQueryQMax = 64;
...
export const SearchTrainsQueryParams = zod.object({
  "q": zod.coerce.string().trim().max(searchTrainsQueryQMax).describe('Search query (e.g. 229, rajdhani, mumbai rajdhani)'),
