...
import { isValidDepartureDate, toApiDate } from "@/lib/validation";
import type { TrainValidationState } from "@/lib/validation";
import { LanguageSwitcher } from "@/components/language-switcher";
import { TrainNumberInput } from "@/components/train-number-input";
import { TrainIllustration } from "@/components/TrainIllustration";

export default function Home() {
  const { t } = useI18n();
  const [trainNo, setTrainNo] = useState("");
  const [trainValidity, setTrainValidity] = useState<TrainValidationState>({ status: "idle" });
  ...
  const dateValid = isValidDepartureDate(depDate);
  const canSubmit = trainValidity.status === "valid" && dateValid;
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (!canSubmit || !trainNo.trim() || !depDate) return;
    setApiParams({
      train_number: trainNo.trim(),
      departure_date: toApiDate(depDate),
    });
    setSearched(true);
  };
      ...
      <div className="min-h-[100dvh] flex flex-col items-center justify-center p-4 md:p-6 bg-background relative overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-md">
          <div className="flex flex-col items-center mb-6">
            ...
            <p className="text-muted-foreground font-mono text-sm mt-2 text-center uppercase">
              {t("app.tagline")}
            </p>
          </div>

          <div className="mb-6 rounded-lg overflow-hidden border border-card-border bg-card/60">
            <TrainIllustration className="h-16 w-full" />
          </div>

          <form onSubmit={handleSearch} className="bg-card/80 backdrop-blur-sm border border-card-border p-6 md:p-8 rounded-lg shadow-xl space-y-6" noValidate>
            ...
            <div className="space-y-2">
              ...
              />
              {!dateValid && (
                <p role="alert" className="mt-1.5 font-mono text-xs uppercase tracking-widest text-destructive" data-testid="error-departure-date">
                  {t("hint.invalidDate")}
                </p>
              )}
            </div>
            <Button
              type="submit"
              disabled={!canSubmit}
              className="w-full text-lg h-14 uppercase tracking-widest mt-4 font-bold border border-primary/50 shadow-[0_0_10px_rgba(0,255,0,0.1)] hover:shadow-[0_0_15px_rgba(0,255,0,0.3)] transition-all disabled:opacity-40"
              data-testid="button-submit-search"
            >
              <Search className="w-5 h-5 me-3" /> {t("action.executeTrace")}
            </Button>
            {trainValidity.status === "invalid" && (
              <p role="alert" className="font-mono text-xs uppercase tracking-widest text-destructive text-center">
                {t("hint.unknownTrain")}
              </p>
            )}
          </form>
        ...
        <div className="max-w-3xl mx-auto px-4 py-4 md:py-6 flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex-1">
            <Button
              ...
              size="sm"
              onClick={reset}
              className="font-mono text-muted-foreground uppercase text-xs hover:text-foreground hover:bg-transparent -ms-2 mb-2"
              data-testid="button-new-search"
            >
              <ArrowLeft className="w-4 h-4 me-2" /> {t("action.abortReturn")}
            </Button>
          ...
          <div className="flex flex-col md:items-end justify-start shrink-0 gap-3">
            <LanguageSwitcher />
            {data && !isLoading && !isFetching && (
              <div className="flex flex-col items-start md:items-end justify-start shrink-0">
                ...
                >
                  {data.current_delay_minutes ? t("status.lateMinutes", { n: data.current_delay_minutes }) : t("status.onTime")}
                </Badge>
                {data.last_updated && (
                  <div className="text-[10px] text-muted-foreground font-mono mt-2 flex items-center gap-1 uppercase tracking-widest">
                    <Clock className="w-3 h-3" />
                    {t("status.updated", {
                      time: new Date(data.last_updated).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                    })}
                  </div>
                )}
              </div>
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto p-4 md:p-8">
        ...
        {isError && !isFetching && (
          <div className="bg-destructive/10 border border-destructive rounded-md p-6 flex flex-col items-center text-center space-y-4">
            ...
            <h2 className="font-mono text-lg text-destructive font-bold uppercase tracking-widest">{t("error.signalLost")}</h2>
            <p className="font-mono text-muted-foreground text-sm max-w-md">
              {(error as any)?.response?.data?.error || t("error.fallback")}
            </p>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4">
                        <div>
                          ...
                          <div className="text-[10px] md:text-xs text-muted-foreground mt-2 flex gap-x-4 gap-y-1 flex-wrap uppercase tracking-widest font-semibold">
                            ...
                            <span>{t("meta.kilometers", { n: station.distance_from_source ?? "-" })}</span>
                            <span>{t("meta.halt", { n: station.halt_minutes ? station.halt_minutes : "-" })}</span>
                            <span>{t("meta.day", { n: station.day })}</span>
                          </div>
                        </div>
                        ...
                        {station.delay_minutes && station.delay_minutes > 0 ? (
                          <Badge variant="outline" className="text-[10px] h-5 py-0 border-warning text-warning shrink-0 mt-1 sm:mt-0 tracking-widest bg-warning/5">
                            {t("status.lateMinutes", { n: station.delay_minutes })}
                          </Badge>
